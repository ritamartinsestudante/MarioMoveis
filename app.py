...
# ---------------------------------------------------------
NOME_BANCO = "controle_comercial.db"
...
def inicializar_banco():
    ...
    try:
        cursor.execute("ALTER TABLE estoque ADD COLUMN estado TEXT DEFAULT 'Novo ✨'")
...
# ---------------------------------------------------------
st.set_page_config(
    page_title="Controle de Gastos Comercial",
    page_icon="💼",
    layout="wide",
...
with aba_vendas:
        ...
        if not produtos_disponiveis.empty:
            with st.form("form_venda", clear_on_submit=True):
                ...
                with col_v1:
                    ...
                    estado_sugerido = item_info.get("Estado (Novo/Usado)", "Novo ✨")
                    ...
                    estado_movel_venda = st.radio("Condição / Estado do Item", ["Novo ✨", "Usado ♻️"],
                ...
                with col_v2:
                    ...
                    custo_frete_venda = st.number_input("Custo de Transporte / Frete para Entrega (R$)", min_value=0.0, value=0.0, step=10.0, format="%.2f", help="Quanto gastou de combustível ou frete para entregar este item ao cliente.")
    ...
    if not df_vendas.empty:
        ...
        vendas_novos = df_vendas[df_vendas["Novo / Usado"] == "Novo ✨"]["Valor Total (R$)"].sum()
        vendas_usados = df_vendas[df_vendas["Novo / Usado"] == "Usado ♻️"]["Valor Total (R$)"].sum()
...
with aba_financeiro:
    ...
    with st.form("form_lancamento", clear_on_submit=True):
        ...
        descricao_obs = st.text_input("Descrição", placeholder="Ex: Pagamento de aluguel / Conta de luz / Frete entrega")
...
with aba_estoque:
    ...
    with st.form("form_add_estoque", clear_on_submit=True):
        nome_produto = st.text_input("Nome do Produto / Material", placeholder="Ex: Produto X / Item Usado Y")
        cat_produto = st.selectbox("Categoria", ["Produto Acabado", "Matéria-Prima", "Acessório", "Insumo / Ferramenta", "Outro"])
        estado_produto = st.selectbox("Condição / Estado do Item", ["Novo ✨", "Usado ♻️", "Não aplicável (Insumo/Ferramenta)"])
        qtd_produto = st.number_input("Quantidade a Adicionar", min_value=1, step=1, value=1)

        preco_custo_unit = st.number_input("Preço de Custo Unitário (R$) [Quanto pagou no item/material]", min_value=0.0, step=10.0, format="%.2f")
        preco_venda_unit = st.number_input("Preço de Venda Unitário (R$) [Por quanto vai vender]", min_value=0.0, step=50.0, format="%.2f")
